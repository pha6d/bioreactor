// This Arduino sketch configures PWM output to control a DC motor pump using a PW1VA01 module (PWM to Voltage converter),
// and controls a relay to power the motor setup. The PWM module converts PWM signals to a 0-5V output
// and is powered by 12V. It connects to the Arduino via pin 5.
...
void controlPump(bool pumpOn, int speedPercentage) {
  if (pumpOn && speedPercentage >= 10 && speedPercentage <= 100) {
    ...
    analogWrite(5, pwmValue);  // Set the speed
    digitalWrite(6, HIGH);     // Turn on the relay to power the pump
    Serial.print("Pump is ON, Speed set to: ");
    Serial.println(speedPercentage);
  } else {
    analogWrite(5, 0);         // Set speed to 0
    digitalWrite(6, LOW);      // Turn off the relay to power down the pump
...
void setup() {
  Serial.begin(115200);
  pinMode(5, OUTPUT); // PWM output for speed control
  pinMode(6, OUTPUT); // Relay control
}
